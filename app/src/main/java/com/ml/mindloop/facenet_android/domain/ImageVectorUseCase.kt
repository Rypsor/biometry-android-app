...
import com.ml.mindloop.facenet_android.data.FaceImageRecord
import com.ml.mindloop.facenet_android.data.ImagesVectorDB
import com.ml.mindloop.facenet_android.data.RecognitionMetrics
import com.ml.mindloop.facenet_android.domain.embeddings.FaceNet
import com.ml.mindloop.facenet_android.domain.face_detection.FaceSpoofDetector
import com.ml.mindloop.facenet_android.domain.face_detection.MediapipeFaceDetector
        ...
        for (result in faceDetectionResult) {
            ...
            if (distance > 0.4) {
                faceRecognitionResults.add(
                    FaceRecognitionResult(recognitionResult.personName, boundingBox, distance, spoofResult),
                )
            } else {
                faceRecognitionResults.add(
                    FaceRecognitionResult("Not recognized", boundingBox, distance, spoofResult),
                )
            }
        }
        val metrics =
