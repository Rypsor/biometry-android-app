...
import com.ml.mindloop.facenet_android.presentation.components.AppAlertDialog
import com.ml.mindloop.facenet_android.presentation.components.DelayedVisibility
import com.ml.mindloop.facenet_android.presentation.components.FaceDetectionOverlay
import com.ml.mindloop.facenet_android.presentation.components.createAlertDialog
...
fun DetectScreen(
    ...
    FaceNetAndroidTheme {
        if (viewModel.recognitionSuccessful.value) {
            SuccessOverlay(
                workerName = viewModel.recognizedWorkerName.value,
                confidence = viewModel.recognitionConfidence.value,
                onNavigateBack = {
            ...
            Scaffold(
                ...
                Column(modifier = Modifier.padding(innerPadding)) { ScreenUI(viewModel, eventType) }
            }
        }
    }
}
...
@Composable
private fun SuccessOverlay(workerName: String, confidence: Float, onNavigateBack: () -> Unit) {
    LaunchedEffect(Unit) {
        delay(1000)
        onNavigateBack()
    }
    ...
    Box(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Text(
                text = "Confianza: %.2f".format(confidence),
                color = Color.White,
                fontSize = 24.sp,
            )
        }
...
@Composable
private fun ScreenUI(viewModel: DetectScreenViewModel, eventType: String) {
    Box {
        Camera(viewModel, eventType)
...
@Composable
private fun Camera(viewModel: DetectScreenViewModel, eventType: String) {
    ...
    DelayedVisibility(cameraPermissionStatus.value) {
        AndroidView(
            modifier = Modifier.fillMaxSize(),
            factory = { FaceDetectionOverlay(lifecycleOwner, context, viewModel, eventType) },
