package com.ml.mindloop.facenet_android.presentation.components
...
import com.ml.mindloop.facenet_android.presentation.screens.detect_screen.DetectScreenViewModel
...
class FaceDetectionOverlay(
    ...
    private val analyzer =
        ImageAnalysis.Analyzer { image ->
            if (isProcessing || viewModel.recognitionSuccessful.value) {
                image.close()
                return@Analyzer
            }
            isProcessing = true
            ...
            CoroutineScope(Dispatchers.Default).launch {
                ...
                results.forEach { (name, boundingBox, confidence, spoofResult) ->
                    val box = boundingBox.toRectF()
                    var personName = name
                    if (viewModel.getNumPeople().toInt() == 0) {
                        personName = ""
                    } else {
                        if (name.isNotEmpty() && name != "Not recognized") { // If a person is recognized
                            viewModel.onRecognitionSuccess(name, eventType, confidence) // Notify the ViewModel
                        }
                    }
